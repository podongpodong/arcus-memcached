...
static void set_cmdlog_path(ms_producer *p)
{
    int64_t lasttime = chkpt_get_lasttime();
    snprintf(p->cmdlog_path, MAX_FILEPATH_LENGTH,
            "%s/%s_%"PRId64"_%06d",
            p->logs_path, "cmdlog", lasttime, 1);
}

static void set_snapshot_path(ms_producer *p)
{
    int64_t lasttime = chkpt_get_lasttime();
...
static void* producer_thread_main(void* arg)
{
    ...
    while(1) {
        ...
        if (p->snapshot_path[0] == '\0') {
            set_snapshot_path(p);
        }

        if (p->cmdlog_path[0] == '\0') {
            set_cmdlog_path(p);
        }
...
ENGINE_ERROR_CODE ms_prod_init(struct default_engine* engine)
{
    ...
    producer.running = RUNNING_STARTED;
