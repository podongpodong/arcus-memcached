...
static int produce_cmdlog(ms_producer *p, int *cmdlog_offset)
{
    ...
    while (1) {
        ...
        if (seek_offset >= file_size) {
            close(fd);
            if (cmdlog_file_deletable()) {
                char path[MAX_FILEPATH_LENGTH];
                next_cmdlog_path(p->cmdlog_path, path);
...
static void set_cmdlog_path(ms_producer *p, int64_t lasttime)
{
    snprintf(p->cmdlog_path, MAX_FILEPATH_LENGTH,
            "%s/%s_%"PRId64"_%06d",
            p->logs_path, "cmdlog", lasttime, 1);
}

static void set_snapshot_path(ms_producer *p, int64_t lasttime)
{
    snprintf(p->snapshot_path, MAX_FILEPATH_LENGTH,
            "%s/%s_%"PRId64,
            p->data_path, "snapshot", lasttime);
}
...
static void* producer_thread_main(void* arg)
{
    ...
    while(1) {
        ...
        newtime = chkpt_get_lasttime();

        if (p->snapshot_path[0] == '\0' || newtime != lasttime) {
            lasttime = newtime;
            set_snapshot_path(p, lasttime);
        }

        if (p->cmdlog_path[0] == '\0'|| newtime != lasttime) {
            lasttime = newtime;
            set_cmdlog_path(p, lasttime);
        }
