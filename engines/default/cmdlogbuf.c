...
static bool do_log_buff_flush_requested(void)
{
    ...
    if (LOGSN_IS_LE(&log_buff_gl.nxt_flush_lsn, &log_buff_gl.upt_flush_lsn)) {
        flush = true;
    }
...
void cmdlog_buff_flush_request(LogSN *upto_lsn)
{
    ...
    pthread_mutex_lock(&log_buff_gl.flush_lsn_lock);
    if (LOGSN_IS_LE(&log_buff_gl.nxt_flush_lsn, upto_lsn) &&
        LOGSN_IS_LE(&log_buff_gl.upt_flush_lsn, upto_lsn)) {
        log_buff_gl.upt_flush_lsn = *upto_lsn;
        requested = true;
    }
    pthread_mutex_unlock(&log_buff_gl.flush_lsn_lock);

    if (requested && log_buff_gl.log_flusher.sleep) {
        do_log_flusher_wakeup(&log_buff_gl.log_flusher);
    }
}
